...
class NFCHandler:
    ...
    def ulc_read_pages(self, start_page, end_page):
        ...
        for page in range(start_page, end_page + 1):
            ...
            result += bytes(resp[:4])  # Each page is 4 bytes
        return result
    ...
    def ulc_authenticate_3des(self, key16):
        ...
        return True  # Simplified: assumes auth success for demo
...
if __name__ == "__main__":
    handler = NFCHandler()
    key16 = bytes.fromhex("49454D4B41455242214E4143554F5946") if '--auth' in sys.argv else None
    
    # Example: Read pages 4 to 6
    try:
        data = handler.read_write_nfc(action="read", start_page=4, end_page=6)
        print(f"Read data: {data.hex()} ({len(data)} bytes)")
    except Exception as e:
        print(f"Error: {e}")

    # Example: Write "TEST" to page 4
    try:
        handler.read_write_nfc(action="write", start_page=4, data=b"TEST", key16=key16)
